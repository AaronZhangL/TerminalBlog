 ...
 */
if (!isset($recipient)) {
  $recipient = 'webmaster@thisdomain.com';
} else {
...
$content["html"] = stripslashes($contentArray["html"]) .  str_replace("\n","<br />",$content['env']);
...
function parse_form($array, $sort = "") {
  ...
  $reserved_keys[] = "fixedFromEmail";
  $reserved_keys[] = "fixedFromName";
  ...
  $reserved_keys[] = "useEnvRpt";
  $reserved_keys[] = "Username";
...
function mail_it($content, $subject, $email, $realname, $recipient, $inbound=true) {
  ...
  if (isset($content["html"]) && $content["html"] != '') {
    ...
    $content["html"] = str_replace("\r",'',$content["html"]);
    $content["html"] = str_replace("\n",'<br />',$content["html"]);
  }
  if ($realname) {
  ...
  $mail_message .= "\n--" . $ob . "--\n";
  if (file_exists($PHPMailerLocation) || file_exists($PHPMailerLiteLocation)) {
    if (file_exists($PHPMailerLiteLocation)) {
      include_once($PHPMailerLiteLocation);
      $mail = new PHPMailerLite();
    } elseif (file_exists($PHPMailerLocation)) {
    ...
    if (isset($_POST['fixedFromEmail'])) {
      ...
      if (stristr($mail->Version,'5.1')) {
        ...
        $mail->From     = $_POST['fixedFromEmail'];
        $mail->FromName = $_POST['fixedFromName'];
      }
    } else {
